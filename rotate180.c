...
int main()
{
  ...
  FILE *fileOut = fopen("D:\\0023_prjo_Embedded_Android\\70_member\\QuanNH\\OutputTraining\\Image\\rotate180.bmp", "wb");
  ...
  for (i = 0; i < width; i++)
  {
    for (j = 0; j < height; j++)
    {
      // imageDataOut[i][j] = imageDataIn[width - 1 - i][j];
      imageDataOut[width - 1 - i][height - 1 - j] = imageDataIn[i][j];
    }
  }
