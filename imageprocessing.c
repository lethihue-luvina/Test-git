...
int main()
{
  ...
  *(int *)&header[22] = temp;
  ...
  fread(buf, sizeof(unsigned char), (height * width), fileIn);

  FILE *fileOut = fopen("D:\\0023_prjo_Embedded_Android\\70_member\\QuanNH\\OutputTraining\\Image\\fileOut.bmp", "wb");
  ...
  fwrite(buf, sizeof(unsigned char), (height * width), fileOut);
